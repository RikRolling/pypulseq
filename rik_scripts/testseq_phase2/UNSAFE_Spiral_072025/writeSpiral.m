...
addpath(genpath('/cubric/data/c24073803/pulseq_matlab/matlab'))
...
fov=250e-3; Nx=64; Ny=Nx;  % Define FOV and resolution
...
clear ka;
ka(kRadius*kSamples+1) = 1i ; % initialize as complex with length kRadius*kSamples+1
...
for c = 0:kRadius*kSamples*tos_calculation/2 % QC: total number of k-space points, account for oversampling for calculation, added step of 2 start:step:end
...
% PNS
[ok, pns_norm, pns_comp, t_axis]=seq.calcPNS('combined_copy.asc');
if (ok)
    fprintf('PNS check passed\n');
else
    fprintf('PNS check failed\n');
end

% Acoustic Frequencies

mr.Siemens.readasc('combined_copy.asc');
dt=sys.gradRasterTime; % time raster
fmax=10000; %10kHz
nwin=5000; % 0.05s
os=3; % frequency oversampling for prettier peaks
ascName=['combined_copy.asc']; % this disables the display of the system's resonance frequences
...
seq.write('spiral_longtime.seq');   % Output sequence for scanner
